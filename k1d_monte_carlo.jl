...
function monte_carlo_sim(data::Dict{String,Vector{Int64}}, keys::Tuple{String, String}, T::Vector{Int64}, m::Int64 = 100)
    ...
    if key1 == key2
        ...
        for i in 2:m
            mc_sample = cumsum(rand(Exponential(lambda_hat), N))
        ...
        for i in 2:m
            mc_sample_x = cumsum(rand(Exponential(lambda_hat_x), Nx))
            mc_sample_y = cumsum(rand(Exponential(lambda_hat_y), Ny))
